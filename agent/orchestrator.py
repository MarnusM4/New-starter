...
from lib.halo import (  # noqa: E402
...
def process_ticket(
    ...
    raw = halo.get_ticket_raw(ticket_id)
    try:
        ticket = halo.parse_ticket(raw)
        ...
        halo.update_status(ticket_id, STATUS_NEEDS_ATTENTION)
    ...
    if ticket.ticket_type is not TicketType.STARTER:
        ...
        halo.post_note(ticket_id, "Leaver tickets are not automated yet (Phase E).")
        return None
    ...
    # Resolve the HALO client to a config via the lookup table. Unknown -> flag a human.
    try:
        config = resolve_config(ticket.client_id)
    except UnknownClientError as exc:
        audit("client.unknown", ticket_id, run_id=run_id, halo_client=ticket.client_id)
        halo.post_note(
            ticket_id,
            f"⚠️ No automation config for HALO client '{ticket.client_id}'. "
            "Add it to clients/_lookup.yaml. No action taken.",
        )
        halo.update_status(ticket_id, STATUS_NEEDS_ATTENTION)
        notify(f"Ticket {ticket_id}: unknown HALO client '{ticket.client_id}' — needs config.")
        return None
    ...
    if config.approval_required and not is_approved(raw):
        if not state.is_done(planned_key(ticket_id)):
            halo.post_note(ticket_id, plan.to_note())
            halo.update_status(ticket_id, STATUS_AWAITING_APPROVAL)
    ...
    try:
        ...
        halo.update_status(ticket_id, STATUS_FAILED)
    ...
    halo.post_note(ticket_id, result_note(result))
    halo.update_status(ticket_id, STATUS_COMPLETED)
