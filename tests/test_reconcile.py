...
def test_reconcile_processes_all_candidates(monkeypatch):
    seen = []
    monkeypatch.setattr(reconcile, "process_ticket", lambda tid, desk, state: seen.append(tid))
    summary = reconcile.run_reconciliation(desk=FakeDesk(["T-1", "T-2"]), state=InMemoryState())
    assert seen == ["T-1", "T-2"]
    assert summary == {"candidates": 2, "processed": 2, "failed": 0}


def test_reconcile_continues_past_a_failing_ticket(monkeypatch):
    def flaky(tid, desk, state):
        if tid == "T-2":
            raise RuntimeError("boom")

    monkeypatch.setattr(reconcile, "process_ticket", flaky)
    summary = reconcile.run_reconciliation(
        desk=FakeDesk(["T-1", "T-2", "T-3"]), state=InMemoryState()
    )
